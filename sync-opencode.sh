...
# Agents: generated by generate-agents.sh from .claude/agents/ (source of
# truth) directly into .opencode/agents — not copied from the root agents/ dir.
if [[ -x "$LIBRARY_DIR/generate-agents.sh" ]]; then
  "$LIBRARY_DIR/generate-agents.sh" >/dev/null
  ((COPIED++)) || true
fi
